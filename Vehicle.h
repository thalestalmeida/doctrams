  ...
  */
  class Vehicle : public ns3::Object
  {
    private: 
      ...
      double m_time;
      double m_time1Ap;
      // Atributo que armazena a distância de segurança proporcional à velocidade
      double m_distSegDin;
      /* testeaviso: mecanismo que impede que veículos que trocaram de pista devido à um obstáculo retornem 
     * à mesma antes de ultrapassar o respectivo obstáculo.*/
      bool m_trocaForcada;
      /* testeaviso: objeto que armazena a posição e pista de um possível obstáculo que se encontra no trecho
       * que o veículo vai entrar. O veículo usa estas informações para saber quando ultrapassou o obstáculo.*/
      Table::obstac m_valObst;
      /* testeaviso: Array que funciona como uma base de dados particular de cada veículo, onde o mesmo armazena
       * a posição e pista de um obstáculo descoberto, para depois enviar para a TCT.*/      
      Table::obstac *m_obst;
      // Fim da adição
      
      // Adicionado por Fabiano
      std::list<Ptr<Table> > m_table;
      // Fim da adição
    ...
    public:
      ...
      */
      ...
      bool GetIsAp();
      void SetIdApAnt(int idApAnt);
      int GetIdApAnt();
      void SetTime (double time);
      double GetTime();
      void SetTime1Ap (double time1Ap);
      double GetTime1Ap();
      void SetTable(int qtdLinhas);
      // Métodos que manipulam a distância de segurança proporcional à velocidade do veículo
      void SetDistSegDin(double value);
      double GetDistSegDin();
      // testeaviso
      void SetObstac(int trecho, double posic, int lane);
      Table::obstac GetObstac(int trecho);
      void SetTrocaForcada(bool value);
      bool GetTrocaForcada();
      void SetValObst(Table::obstac value);
      void SetValObst(double pos, int pista);
      Table::obstac GetValObst();
      // Fim da adição
      
      // Adicionado por Fabiano
      void AddTable(Ptr<Table> table);
      std::list<Ptr<Table> > GetTable();
      // newttl
      //void UpdateTable(std::list<Ptr<Table> > table, int trecho, double decrem);
      void UpdateTable(std::list<Ptr<Table> > table, int trecho, int trechoProp, int trechoIniDir, int trechoFimDirCont, int treFimDir, double decrem, double ttlTreAtual);
	  // Fim da adição
